...
final class DefaultChannelOutboundBuffer implements ChannelOutboundBuffer {
    ...
    private static final AtomicIntegerFieldUpdater<DefaultChannelOutboundBuffer> WRITABLE_UPDATER;

    @SuppressWarnings("FieldMayBeFinal")
    private volatile int writable = 1;

    static {
        AtomicIntegerFieldUpdater<DefaultChannelOutboundBuffer> writableUpdater =
                PlatformDependent.newAtomicIntegerFieldUpdater(DefaultChannelOutboundBuffer.class, "writable");
        if (writableUpdater == null) {
            writableUpdater = AtomicIntegerFieldUpdater.newUpdater(DefaultChannelOutboundBuffer.class, "writable");
        }
        WRITABLE_UPDATER = writableUpdater;

        AtomicLongFieldUpdater<DefaultChannelOutboundBuffer> pendingSizeUpdater =
                PlatformDependent.newAtomicLongFieldUpdater(DefaultChannelOutboundBuffer.class, "totalPendingSize");
        if (pendingSizeUpdater == null) {
            pendingSizeUpdater = AtomicLongFieldUpdater.newUpdater(
                    DefaultChannelOutboundBuffer.class, "totalPendingSize");
        }
        TOTAL_PENDING_SIZE_UPDATER = pendingSizeUpdater;
    }

    DefaultChannelOutboundBuffer(AbstractChannel channel) {
        this.channel = channel;
    }

    @Override
    public void addMessage(Object msg, ChannelPromise promise) {
